...
static AST_T* builtin_function_exit(visitor_T* visitor, AST_T** args, int args_size)
{
    for (int i = 0; i < args_size; i++)
    {
        ...
        switch (visited_ast->type)
        {
            case AST_NOOP: printf("Peace out!✌️\n"); exit(0); break;
            default: printf("%p\n", visited_ast); break;
        }
    }
...
AST_T* visitor_visit(visitor_T* visitor, AST_T* node)
{
	...
	printf("huh 🤔, uncaught statement of type '%d'\n", node->type);
...
AST_T* visitor_visit_function_call(visitor_T* visitor, AST_T* node)
{
	...
	{
		// grab variable from function definiiton arguments
		AST_T* ast_var = (AST_T*) fdef->function_definition_args[i];
		//grab the value from the functioncall arguments
		AST_T* ast_value = (AST_T*) node->function_call_arguments[i];
		// create variable definition
		AST_T* ast_vardef = init_ast(AST_VARIABLE_DEFINITION);
		// create a new variable definition with the value of the argument in the new function call
		...
		// push the variable definition into the function definition body toscope 
