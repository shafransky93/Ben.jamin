...
void parser_eat(parser_T* parser, int token_type)
{
	...
	{
		printf(
			"🤔 I dont know what to do with token '%s', it has type '%d' token",
...
AST_T* parser_parse(parser_T* parser) // main entry point
{
...
AST_T* parser_parse_statements(parser_T* parser)
{
	...
	while (parser->current_token->type  == TOKEN_SEMI) // keep parsing if : exists
	{
...
AST_T* parser_parse_function_call(parser_T* parser)
{
	...
	function_call->function_call_arguments +=1;
			
	while (parser->current_token->type  == TOKEN_COMA) // keep parsing if , exists
	{
		parser_eat(parser, TOKEN_COMA);
