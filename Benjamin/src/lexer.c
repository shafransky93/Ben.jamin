...
{
	lexer_T* lexer = calloc(1, sizeof(struct LEXER_STRUCT)); // allocate memory
...
{
	if (lexer->c != '\0' && lexer->i < strlen(lexer->contents)) // increment characters
	{
		lexer->i += 1;
...
{
	while (lexer->c == ' ' || lexer->c == 10) //skip white space and value is for new line
	{
		lexer_advance(lexer);
	}
}
...
token_T* lexer_get_next_token(lexer_T* lexer)
{
	while (lexer->c != '\0' && lexer->i < strlen(lexer->contents))
	{
		...
		switch (lexer->c)
		{
			...
			case ',': return lexer_advance_with_token(lexer, init_token(TOKEN_COMA,lexer_get_current_char_as_string(lexer))); break;
